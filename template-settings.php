...
class Dokan_Template_Settings {
    ...
    function insert_settings_info() {
        ...
        if ( wp_verify_nonce( $_POST['_wpnonce'], 'dokan_profile_settings_nonce' ) ) {
            ...
            ////////////////create array for hubtel withdrawal forms/////
            if (isset($_POST['settings']['hubtel'])) {
                $hubtel = $_POST['settings']['hubtel'];


                $dokan_settings['payment']['hubtel'] = array (
                'email'         => filter_var( $hubtel['email'], FILTER_VALIDATE_EMAIL ),
                'fullname'      => sanitize_text_field( $hubtel['fullname'] ),
                'phone_number'   => sanitize_text_field( $hubtel['phone_number'] ),
                'Channel'   => sanitize_text_field( $hubtel['Channel'] ),
                );
