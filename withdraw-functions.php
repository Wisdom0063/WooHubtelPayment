...
function dokan_withdraw_register_methods() {
    $methods = array(
        ...
        'hubtel' =>array(
            'title'    => __( 'Hubtel Mobile Payment', 'dokan-lite' ),
            'callback' => 'dokan_withdraw_method_hubtel'
        )
    );
...
/**
 * Callback for Hubtel Payment in store settings
 *
 * @global WP_User $current_user
 * @param array $store_settings
 */
function dokan_withdraw_method_hubtel( $store_settings ) {
    global $current_user;
 


        $email = isset( $store_settings['payment']['hubtel']['email'] ) ? esc_attr( $store_settings['payment']['hubtel']['email'] ) : $current_user->user_email ;
        $phone_number = isset($store_settings['payment']['hubtel']['phone_number']) ? esc_attr( $store_settings['payment']['hubtel']['phone_number']): '';
        $fullname = isset( $store_settings['payment']['hubtel']['fullname'] ) ? esc_attr( $store_settings['payment']['hubtel']['fullname'] ): '';
        $network = isset( $store_settings['payment']['hubtel']['Channel'] ) ? esc_attr( $store_settings['payment']['hubtel']['Channel'] ): '';
        ?>
        <div class="dokan-form-group">
            <div class="dokan-w8">
                <div class="dokan-input-group">
                    <span class="dokan-input-group-addon"><?php _e( 'E-mail', 'dokan-lite' ); ?></span>
                    <input value="<?php echo $email; ?>" name="settings[hubtel][email]" class="dokan-form-control email" placeholder="name@dmain.com" type="text" required>
                </div>
            </div>
        </div>
        ...
        <div class="dokan-w8">
        <div class="dokan-input-group">
        <span class="dokan-input-group-addon">Full name</span>
            <input name="settings[hubtel][fullname]"  value="<?php echo $fullname; ?>" class="dokan-form-control" placeholder="<?php esc_attr_e( 'Your full name', 'dokan-lite' ); ?>" type="text" required>
       </div>
        </div>
    </div>
        ...
        <div class="dokan-form-group" >
            <div class="dokan-w8">
                <div class="dokan-input-group">
                    <span class="dokan-input-group-addon"><?php _e( 'Phone Number', 'dokan-lite' ); ?> </span>
                    <input value="<?php echo $phone_number ; ?>" name="settings[hubtel][phone_number]" class="dokan-form-control number" placeholder="0241676522" type="number" required>
                </div>
            </div>
        </div>
        <div class="dokan-form-group" >
            <div class="dokan-w8">
                <div class="dokan-input-group">
                    <span class="dokan-input-group-addon"><?php _e( 'Network Provider', 'dokan-lite' ); ?> </span>
<script type='text/javascript'>
  $(document).ready(function(){
      var channel = '<?php echo $network?>';
      if(channel){
        $('#Channel option[value=<?php echo $network?>]').attr('selected','selected');
      }

  });
</script> 
<select name="settings[hubtel][Channel]" required="required"  id='Channel'  >
...
function dokan_get_seller_active_withdraw_methods() {
    ...
    $hubtel = isset( $payment_methods[0]['payment']['hubtel']['phone_number'] ) && $payment_methods[0]['payment']['hubtel']['fullname']  !== '' ? 'hubtel' : '';

    $payment_methods = array( $paypal, $bank, $skrill,$hubtel );
